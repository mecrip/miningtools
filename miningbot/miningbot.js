...
function getNvidiaReport(callback) {
  exec("nvidia-smi --query-gpu=name,utilization.gpu,temperature.gpu,power.draw,power.limit,fan.speed --format=csv", function(error, stdout, stderr){ callback(stdout); });
};
...
function QueryETC(callback,w){
  ...
  //get balance
  http.get(`https://api.gastracker.io/addr/${w.wallet}`, (resp) => {
    let data = '';
    ...
    resp.on('end', () => {
      result.balance=JSON.parse(data).balance.ether;
...
function QueryVTC(callback,w){
  ...
  http.get(`http://explorer.vertcoin.info/ext/getbalance/${w.wallet}`, (resp) => {
    ...
    // The whole response has been received. Print out the result.
    resp.on('end', () => { result.balance=data;
